...
| SECURITY-15 (Exception handling/fail-safe defaults) | **Compliant** | `enviarAvaliacao`'s write follows the same existing try/catch-and-toast pattern used by every other client write in this codebase; a failed write leaves the obra's rating fields absent, so the UI gate (`!obra.avaliacaoCriterios`) naturally fails closed (button stays visible, no partial-rated state is representable). |
