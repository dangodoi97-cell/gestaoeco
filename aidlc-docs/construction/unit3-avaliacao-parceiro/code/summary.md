...
## Post-Deploy Enhancement (2026-07-09) — Notification Wording + Obra Detail Display
- `cliente/app.js` — `enviarAvaliacaoObra`: notification `titulo` for the admin changed from "Nova avaliação de {nome}" to "Obra avaliada".
- `admin/app.js` — `renderNotificacoesAdmin`: added an explicit "Conferir" button on `avaliacao_registrada` notification cards (in addition to the whole card already being clickable).
- `admin/app.js` — `renderDetalheObra`: added a new section showing that specific obra's 3 criteria + Geral + comentário (when rated), right after the progress bar. Distinct from the parceiro's *accumulated* average shown on the parceiro detail screen (FR-4.8) — this shows the single rating tied to this one obra.
- **Why**: user request, after testing the notification → obra flow and finding the obra detail screen showed nothing once you got there.
- `npm test` re-run after these changes: 27/27 passing (no regression — both changes are presentation-only, no logic in `js/avaliacao.js` touched).

## Post-Deploy Correction #2 (2026-07-09) — Silent Failure on Submit
- `cliente/app.js` — `enviarAvaliacaoObra`: wrapped `enviarAvaliacao` in try/catch (shows an error toast and keeps the modal open on failure, instead of throwing silently — nothing happened on screen before this fix) and wrapped the `criarNotificacao` admin-notify call in its own try/catch (non-blocking — a notify failure no longer prevents the success toast for an already-saved rating). **Why**: user reported clicking "Enviar avaliação" appeared to do nothing. Root cause: no error handling existed at all in this function (an inherited pattern from the pre-Unit-3 code, which this unit's NFR Requirements had incorrectly characterized as "Compliant" for SECURITY-15 — it wasn't; there was no try/catch anywhere in this flow). **Most likely trigger for the reported case**: `firestore.rules` was pushed to the `dev` git branch but — per this project's manual-deploy convention (`LEIA-ME.md` PASSO 1) — was never republished to the live Firebase project, so the live rules still only accept the old `avaliacaoNota` shape and reject the new `avaliacaoCriterios`/`avaliacaoGeral`/`avaliacaoParceiros` write with a permission-denied error, which this fix now surfaces to the user instead of swallowing. **Action required by the user, outside this session's reach**: republish `firestore.rules` (Firebase Console or `firebase deploy --only firestore:rules`) before this feature can work end-to-end.
